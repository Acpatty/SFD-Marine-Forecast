...
.noaa-text {white-space: pre-wrap; line-height: 1.6; font-size: 0.95rem; margin-top: 10px;}
</style>
...
# WMO Weather Code mapping (standard)
WMO_CODES = {
    0: "Clear sky",
    1: "Mainly clear",
    2: "Partly cloudy",
...
# Fetch NOAA text forecast (same improved parsing)
@st.cache_data(ttl=3600)
...
# Fetch Open-Meteo Atmospheric + Weather Code
@st.cache_data(ttl=3600)
...
# Period summary with condition description
def period_summary(df, start_h, end_h):
    period_df = df[(df['time'].dt.hour >= start_h) & (df['time'].dt.hour < end_h)]
    if period_df.empty:
        return {k: "N/A" for k in ['condition', 'wave', 'wind_speed', 'wind_dir', 'gust', 'precip', 'vis', 'temp']}
    # Dominant condition
...
# Display (updated with Conditions)
# ... Alerts and Tides same
...
for i, (period_name, start_h, end_h) in enumerate(time_periods):
    with cols[i % 2]:
        ...
        st.write(f"**Conditions (Aggregate)**: {summary['condition']}")
        ...
        if i < len(noaa_periods):
            ...
            formatted_text = noaa_text.replace('. ', '.<br>')
            st.markdown(f"<div class='noaa-text'><strong>NOAA Details:</strong><br>{formatted_text}</div>", unsafe_allow_html=True)
        else:
            st.write("_Detailed NOAA text unavailable._")
        
        st.markdown("</div>", unsafe_allow_html=True)

# Footer
st.caption("Conditions & Numerical: Open-Meteo multi-model aggregate | Textual: NOAA | Stay safe!")
